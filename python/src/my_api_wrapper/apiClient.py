...
class PlanetpositionsAPIClientError(Exception):
...
class ValidationError(PlanetpositionsAPIClientError):
...
class PlanetpositionsAPIClient:
    ...
    VALIDATION_RULES = {"date": {"type": "string", "required": False, "format": "date"}, "lat": {"type": "number", "required": True, "min": -90, "max": 90}, "lon": {"type": "number", "required": True, "min": -180, "max": 180}, "alt": {"type": "number", "required": False, "min": 0, "default": 0}, "planet": {"type": "string", "required": True}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the PlanetpositionsAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises PlanetpositionsAPIClientError: If API key is invalid
        """
    ...
    def _validate_api_key(self, api_key: str) -> None:
        ...
        if not api_key or not api_key.strip():
            raise PlanetpositionsAPIClientError(
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[PlanetpositionsAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        :raises PlanetpositionsAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise PlanetpositionsAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise PlanetpositionsAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise PlanetpositionsAPIClientError(f"Request failed: {str(e)}")
